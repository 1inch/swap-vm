...
import { XYCConcentrateSwap } from "./XYCConcentrate.sol";
import { InstructionArgs } from "../libs/InstructionArgs.sol";
import { InstructionBuilder } from "../libs/InstructionBuilder.sol";
import { MemoryPtr, MemoryPtrLib } from "../libs/MemoryPtr.sol";

library XYCDriftingConcentrateSwapLib {
...
contract XYCDriftingConcentrateSwap {
    ...
    function _xycDriftingConcentrate2D(Context memory ctx, bytes calldata args) internal {
        XYCDriftingConcentrateSwapLib.Args calldata config = XYCDriftingConcentrateSwapLib.parse(args);

        // Immutable geometric half-width g = sqrt(sqrtPmax0/sqrtPmin0), 1e18 fp (> 1e18: parse validates the bounds)
        uint256 g = Math.sqrt(Math.mulDiv(config.sqrtPriceMax, XYCConcentrateSwap.ONE * XYCConcentrateSwap.ONE, config.sqrtPriceMin));
        ...
        if (state.lastUpdate != 0 && block.timestamp > state.lastUpdate) {
            ...
            uint256 centerTarget = bLt == 0 ? type(uint256).max : Math.sqrt(Math.mulDiv(bGt, XYCConcentrateSwap.ONE * XYCConcentrateSwap.ONE, bLt));
            uint256 distance = centerTarget > center ? centerTarget - center : center - centerTarget;

            uint256 totalMove = Math.min(distance, Math.mulDiv(center, _driftCap(config.driftRatePerSecond, elapsed), XYCConcentrateSwap.ONE));
            uint256 freeMove = Math.min(totalMove, Math.mulDiv(center, _driftCap(config.baseDriftRatePerSecond, elapsed), XYCConcentrateSwap.ONE));
        ...
        uint256 sqrtPriceMin = Math.mulDiv(center, XYCConcentrateSwap.ONE, g);
        uint256 sqrtPriceMax = Math.mulDiv(center, g, XYCConcentrateSwap.ONE, Math.Rounding.Ceil);
        ...
        {
            uint256 liquidity = XYCConcentrateSwap.computeLiquidity(bLt, bGt, sqrtPriceMin, sqrtPriceMax);
            if (isTokenInLt) {
                virtualBalanceIn  = ctx.swap.balanceIn  + Math.mulDiv(liquidity, XYCConcentrateSwap.ONE, sqrtPriceMax, Math.Rounding.Ceil);
                virtualBalanceOut = ctx.swap.balanceOut + Math.mulDiv(liquidity, sqrtPriceMin, XYCConcentrateSwap.ONE);
            } else {
                virtualBalanceIn  = ctx.swap.balanceIn  + Math.mulDiv(liquidity, sqrtPriceMin, XYCConcentrateSwap.ONE, Math.Rounding.Ceil);
                virtualBalanceOut = ctx.swap.balanceOut + Math.mulDiv(liquidity, XYCConcentrateSwap.ONE, sqrtPriceMax);
            }
        }
    ...
    {
        uint256 deltaPaid = Math.mulDiv(paidMove, XYCConcentrateSwap.ONE, center);
        uint256 cost = 2 * deltaPaid;
        if (cost <= budget) return (paidMove, budget - cost);
        return (Math.mulDiv(center, budget / 2, XYCConcentrateSwap.ONE), 0);
    }
}
