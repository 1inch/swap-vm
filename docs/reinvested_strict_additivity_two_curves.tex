...
We present paper-checkable numbers comparing constant product, the conservative design, and the dissipative design.
...
\paragraph{Conservative forward $X\to Y$.}
\[
...
\paragraph{Conservative round trip.}
...
\paragraph{Dissipative round trip (spread).}
After forward, pool is $(x_1,y_1)=(1100,926.5862513559)$ and trader holds $\Delta y$.
Swap back using dissipative reverse rule with input $\dy=\Delta y$ so $y_2=1000$ and
\[
...
\]
so the trader loses about $34.9071\,X$ on the round trip and the pool gains the same amount (dissipation).

\subsection{Example 2: $x=y=1000$, $\dx=100$, $\alpha=0.997$ (near CP)}

\paragraph{Conservative/Dissipative forward.}
\[
