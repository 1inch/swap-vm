...
> **Auditors & automated scanners — read first:** A number of behaviors in this codebase have been acknowledged or won't-fixed across 8 prior audit rounds as deliberate design properties (multi-`ship()` for the same `strategyHash`, empty token arrays in `ship`/`dock` accepted, `rawBalances` does not validate strategy membership while `safeBalances` does, fee-on-transfer / rebasing tokens explicitly unsupported, maker-declared liquidity not enforced on-chain, and others). Before flagging any of these as bugs, consult [`AquaVM_Auditor_Brief.md`](AquaVM_Auditor_Brief.md): it contains the Responsibility Matrix (on-chain vs delegated), the topic-indexed table of accepted behaviors (§6.2), and the symbol-indexed table keyed by Solidity function name (§6.3).
