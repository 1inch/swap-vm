...
/**
 * @title XYCSwapFlatFeeInInvariants
 * @notice Tests invariants for XYCSwap combined with flat fee on input
 * @dev Tests XYC swap behavior with flat fee applied to input amount
 */
contract XYCSwapFlatFeeInInvariants is Test, OpcodesDebug, CoreInvariants {
    ...
    /**
     * Test XYC + FlatFeeIn with equal balances (1:1 ratio)
     */
    function test_XYCFlatFeeInBasic() public {
    ...
    /**
     * Test XYC + FlatFeeIn with different balance ratios
     */
    function test_XYCFlatFeeInDifferentBalances() public {
        uint32 feeBps = 0.003e9; // 0.3% fee
        ...
        for (uint256 i = 0; i < 4; i++) {
            _testXYCFlatFeeInWithBalances(balancesA[i], balancesB[i], feeBps);
        }
    }

    /**
     * Test XYC + FlatFeeIn with different fee percentages
     */
    function test_XYCFlatFeeInDifferentFees() public {
        uint256 balanceA = 1000e18;
        uint256 balanceB = 1000e18;
        ...
        for (uint256 i = 0; i < 4; i++) {
            _testXYCFlatFeeInWithBalances(balanceA, balanceB, fees[i]);
        }
    }

    /**
     * Test XYC + FlatFeeIn with large liquidity pool
     */
    function test_XYCFlatFeeInLargeLiquidity() public {
    ...
    /**
     * Test XYC + FlatFeeIn with small liquidity pool
     */
    function test_XYCFlatFeeInSmallLiquidity() public {
    ...
    /**
     * Helper to test XYC + FlatFeeIn with specific balances and fee
     */
    function _testXYCFlatFeeInWithBalances(uint256 balanceA, uint256 balanceB, uint32 feeBps) private {
        Program memory program = ProgramBuilder.init(_opcodes());
        bytes memory bytecode = bytes.concat(
            program.build(_dynamicBalancesXD,
                BalancesArgsBuilder.build(
