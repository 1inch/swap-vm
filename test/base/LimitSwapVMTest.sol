...
import { LimitStrategyBuilders } from "./LimitStrategyBuilders.sol";

contract LimitSwapVMTest is LimitStrategyBuilders {
    struct SwapProgram {
        uint256 amount;
        MockTaker taker;
        TokenMock tokenA;
    ...
    constructor() LimitStrategyBuilders(address(aqua)) {}
    ...
    function _deployRouter() internal virtual returns (SwapVM) {
        return new LimitSwapVMRouter(address(aqua), address(0), address(this), "SwapVM", "1.0.0");
    }
