...
library SplineSwapArgsBuilder {
    ...
    /// @dev Packed layout (128 bytes total):
    ...
    ///   - sellDensitySelector: bytes4 (4 bytes) - Density strategy selector for sell
    ///   - buyDensitySelector: bytes4 (4 bytes) - Density strategy selector for buy
    ///   - sellPriceFormulaSelector: bytes4 (4 bytes) - Price formula selector for sell
    ///   - buyPriceFormulaSelector: bytes4 (4 bytes) - Price formula selector for buy
    struct Args {
    ...
    // Packed encoding: 3 * uint256 (96 bytes) + 6 * uint16 (12 bytes) + 4 * bytes4 (16 bytes)
    // Total: 96 + 12 + 16 = 124 bytes (fits in uint8 limit of 255)
    uint256 private constant ARGS_LENGTH = 124;

    /// @notice Build encoded arguments for SplineSwap instruction
    /// @param args The arguments struct
    /// @return Encoded bytes for the instruction (124 bytes)
    function build(Args memory args) internal pure returns (bytes memory) {
        return abi.encodePacked(
            ...
            args.sellDensitySelector,        // 4 bytes (offset 108)
            args.buyDensitySelector,         // 4 bytes (offset 112)
            args.sellPriceFormulaSelector,   // 4 bytes (offset 116)
            args.buyPriceFormulaSelector     // 4 bytes (offset 120)
        );
        // Total: 124 bytes
    }
    ...
    function parse(bytes calldata data) internal pure returns (Args memory args) {
        ...
        // Parse bytes4 selectors
        args.sellDensitySelector = bytes4(data[108:112]);
        args.buyDensitySelector = bytes4(data[112:116]);
        args.sellPriceFormulaSelector = bytes4(data[116:120]);
        args.buyPriceFormulaSelector = bytes4(data[120:124]);
...
/// @title SplineSwap - Configurable curve swap instruction for SwapVM
/// @notice Implements swap logic using configurable density strategies and price formulas
/// @dev Supports 12 density strategies and 9 price formulas with asymmetric buy/sell configuration
contract SplineSwap {
    ...
    /// @notice Main instruction: SplineSwap with configurable curves
    /// @param ctx Swap context containing balances and amounts
    /// @param args Encoded SplineSwap configuration (124 bytes)
    /// @dev Supports both ExactIn and ExactOut modes
    /// @dev Handles boundary crossing between sell and buy regions
    function _splineSwapGrowPriceRange2D(Context memory ctx, bytes calldata args) internal pure {
        ...
        // ╔═══════════════════════════════════════════════════════════════════════════╗
        // ║  SPLINE SWAP - CONFIGURABLE CURVE AMM                                     ║
        // ║                                                                           ║
        // ║  Position State:                                                          ║
        // ║    netPosition = Σ token0_sold_by_maker - Σ token0_bought_by_maker        ║
        // ║    - pos > 0: Sell region (maker has distributed token0)                  ║
        // ║    - pos < 0: Buy region (maker has accumulated token0)                   ║
        // ║    - pos = 0: Initial state                                               ║
        // ║                                                                           ║
        // ║  Price Formula: P = P₀ · formula(f(x)) where f(x) is density curve        ║
        // ║                                                                           ║
        // ║  Supports:                                                                ║
        // ║    - 12 density strategies (Uniform, Quadratic, Stable, etc.)             ║
        // ║    - 9 price formulas (Spline, ConstantProduct, Exponential, etc.)        ║
        // ║    - Asymmetric buy/sell curves                                           ║
        // ║    - Region-based 4-fee spread system                                     ║
        // ╚═══════════════════════════════════════════════════════════════════════════╝

        // In the context of SwapVM:
        // - tokenIn is what the taker provides (maker receives)
        // - tokenOut is what the taker receives (maker provides)
        // - For this instruction, we assume token0 = tokenOut, token1 = tokenIn
        // - When taker buys token0 (tokenOut), maker is selling - use sell side curves
        
        if (ctx.query.isExactIn) {
            require(ctx.swap.amountOut == 0, SplineSwapRecomputeDetected());
            
            // Taker is providing exact amountIn, we need to calculate amountOut
            // Taker is selling tokenIn for tokenOut (token0)
            // From maker's perspective: receiving tokenIn (token1), giving tokenOut (token0)
            // So maker is SELLING token0 -> use sell side parameters
            ...
            // Taker wants exact amountOut, we need to calculate amountIn
    ...
    /// @dev Get average price for a swap, handling region detection
    function _getAverageSwapPrice(
        ...
        // For sell side (maker selling token0):
        // - capacity = token0ToSell
        // - usedCapacity = capacity - balanceOut (how much has already been sold)
        
        uint256 capacity = config.token0ToSell;
        uint256 rangeBps = config.sellRangeBps;
        uint256 spreadBps = config.sellAskBps;
        bytes4 densitySelector = config.sellDensitySelector;
        bytes4 priceSelector = config.sellPriceFormulaSelector;
        ...
        uint256 swapAmount;
        if (isExactIn) {
            // For ExactIn, we estimate token0 amount based on price (iterative approach simplified)
            // Use approximation: amountOut ≈ amountIn / basePrice
